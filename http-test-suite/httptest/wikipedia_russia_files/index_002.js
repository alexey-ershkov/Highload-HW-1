...
jQuery(function ($) {
        ...
        switch (d[0]) {
            case 'request' :
                ...
                for (i = 0; i < coord_list.length; ++i) {
                    ...
                    if (coord_list[i].lat < minlat) {
                        minlat = coord_list[i].lat;
                    }
                    if (coord_list[i].lat > maxlat) {
                        maxlat = coord_list[i].lat;
                    }
                    geoext[i] = {
                ...
                for (i = 0; i < geoext.length; ++i) {
                    ...
                    if (s < 0 && (ineg < 0 || geoext[i].z < geoext[ineg].z)) {
                        ineg = i;
                    }
                    if (s > 0 && (ipos < 0 || geoext[i].z < geoext[ipos].z)) {
                        ipos = i;
                    }
                }
                if (ipos >= 0 && ineg >= 0) {
    ...
    bodyc.find('a.external.text').each(function (key, link) {
        ...
        if (/_globe:([^_&]+)/.test(coord_params)) {
            globe = capitalize(RegExp.$1);
        }
        if ($.inArray(globe, ["Earth", "Moon", "Mars", "Venus", "Mercury", "Io", "Titan"]) < 0) {
            return;
        }
        ...
        mapbutton.bind('click', {
            param:
                marker.lat + '_' + marker.lon + '_' +
                wc.width + '_' + wc.height + '_' +
                site + '_' + zoomlevel + '_' + language + '&globe=' + globe
        }, showIFrame);
        ...
        coord_list.push({
            ...
            obj: link,
            mb: mapbutton,
            title: params.title || params.pagename || ''
        });
    }); //end each
            ...
            wi.div.append(
                $('<div/>')
                    ...
                    .mouseenter(function (e) {
                        wi.resizebutton.fadeIn()
                    })
                    .mouseleave(function (e) {
                        if (idle) {
                            wi.resizebutton.fadeOut();
                        }
                    })
