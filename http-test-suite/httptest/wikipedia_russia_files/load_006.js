mw.loader.implement("ext.vector.footerCleanup", function () {
    (function ($) {
        $.fn.footerCollapsibleList = function (config) {
            ...
            return this.each(function () {
                ...
                $icon = $('<span>');
                $ul.before($('<a>').addClass('collapsible-list').text(config.title).append($icon).on('click', function (e) {
    ...
    (function ($) {
        window.onload = function () {
            if ('wikiEditor' in $) {
                ...
                $cancelLink.parent().empty().append(
                    $cancelLink);
