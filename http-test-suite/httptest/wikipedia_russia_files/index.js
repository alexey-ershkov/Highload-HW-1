...
window.pg || $(document).ready(function ($) {
    ...
    if (wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help') {
        ...
        if (settings[0] == 0) {
            ...
            footer.append($("<li>").append($("<a>").text("Enable Reference Tooltips").attr("href", "javascript:(function(){})()").click(function () {
                toggleRT(1)
            })));
            return;
        }
            ...
            function openSettingsMenu() {
                if (settingsMenu) {
                    ...
                    settingsMenu = $("<form>").append(
                        $("<button>").css("width", "100%").text("Disable Reference Tooltips").button().click(function () {
                            toggleRT(0)
                        }),
                        $("<br>"),
                        $("<small>").text("Once disabled, Reference Tooltips can be re-enabled using a link in the footer of the page."),
                        $("<hr>"),
                        $("<label>").text("Delay before the tooltip appears (in milliseconds): ").append($("<input>").attr({
                            "type": "number",
                            "value": settings[1],
                            step: 50,
                            min: 0,
                        ...
                        modal: true, width: 500, title: "Reference Tooltips options", buttons: {
